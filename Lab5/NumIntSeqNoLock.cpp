...
public:
	NumIntSeq()
		: myH(0), myN(0), myFrom(0), myTo(0), mySum(0), sum_ptr(nullptr), myThread() {}

	NumIntSeq(double h, int n, int from, int to, double* sum)
		: myH(h), myN(n), myFrom(from), myTo(to), mySum(0), sum_ptr(sum), myThread() {}
...
int main(int argc, char* argv[])
{
	...
	NumIntSeq* threads = new NumIntSeq[numThreads];
	...
	for (int i = 0; i < numThreads; ++i)
	{
		...
		threads[i] = NumIntSeq(h, n, from, to, &sum);
		threads[i].start();
	}
