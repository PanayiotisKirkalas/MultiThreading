...
public:
	CounterThread()
		:myThread(), array_ptr(nullptr), counter_ptr(nullptr), myEnd(-1) {}

	CounterThread(int end, int *array, int *counter)
		:myThread(), array_ptr(array), counter_ptr(counter), myEnd(end) {}
...
int main() {
	...
	for (int i = 0; i < numThreads; i++) {
		threads[i] = CounterThread(end, array, &counter);
		threads[i].start();
	}
