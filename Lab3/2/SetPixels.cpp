...
public:
	...
	MySetPixelsThread(vector<unsigned char>* data, int from, int to, int stride, int padding)
	{
		myfrom = from;
		myto = to;
		...
		myThread = thread([this]() {
			for (int i = myfrom; i < myto; ++i) {
				for (int j = 0; j < width; ++j) {
					...
					pixel[0] = (pixel[0] + 100) % 256;
					pixel[1] = (pixel[1] + 100) % 256;
					pixel[2] = (pixel[2] + 100) % 256;
...
void main() {
	...
	// thread creation
	MySetPixelsThread* threads = new MySetPixelsThread[numThreads];
	...
	for (int i = 0; i < numThreads; ++i)
	{
		...
		threads[i] = MySetPixelsThread(&buffer, from, to, stride, padding);
		threads[i].start();
	}
